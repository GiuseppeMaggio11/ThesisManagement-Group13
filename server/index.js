...
app.get(
  "/api/getAllFiles/:student_id/:thesis_id",
  isProfessor,
  async (req, res) => {
    try {
      ...
      zipdir(userFolderPath, { saveTo: zipFilePath }, function (err, buffer) {
        if (err) {
          res
            .status(500)
            .send("An error occurred while creating the zip archive.");
        } else {
...
app.get(
  "/api/getFile/:student_id/:thesis_id/:file_name",
  isProfessor,
  async (req, res) => {
    try {
      const studentID = req.params.student_id;
...
app.get("/api/listExternalCosupervisors", isProfessor, (req, res) => {
  dao
    .getExternal_cosupervisors()
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (external_cosupervisors_emails.includes(req.body.email)) {
        return res.status(400).json({
          error: `External cosupervisor email: ${req.body.email} is already present in db`,
        });
      }
      ...
      const result_external_cosupervisor =
        await dao.create_external_cosupervisor(external_cosupervisor);
...
// Activate the server only if not running in test mode
...
module.exports = { app, isProfessor, isLoggedIn };
