...
async function updateThesis (req,res) {
  ...
  try {
    ...
    const result_thesis_group = await dao.updateThesisGroup(thesis.thesis_id, req.body.cod_group);
    ...
    if (req.body.cosupervisors_internal != null) {
      for (const internal_cosupervisor of req.body.cosupervisors_internal) {
        const result_cosupervisors_internal = [];
        result_cosupervisors_internal.push(await dao.createThesis_cosupervisor_teacher(thesis.thesis_id, internal_cosupervisor))
      }
    }
    ...
    if (req.body.cosupervisors_external != null) {
      for (const external_cosupervisor of req.body.cosupervisors_external) {
        const result_cosupervisors_external = [];
        result_cosupervisors_external.push(await dao.createThesis_cosupervisor_external(thesis.thesis_id, external_cosupervisor))
      }
    }
...
module.exports = {newThesis, updateThesesArchivation, updateThesis}