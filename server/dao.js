  ...
  return new Promise((resolve, reject) => {
    ...
    connection.query(sql, (error, results, fields) => {
      if (error) {
        ...
        if (user_type === "STUD") {
          thesisFromSameDegreeOfStudent = results.filter(
            (item) => item.title_degree === studentTitleDegree
          );
        }
        ...
        const addcosupervisorsArrayToResults =
          thesisFromSameDegreeOfStudent.map((item) => {
  ...
  return new Promise((resolve, reject) => {
            ...
            (error, result2, fields) => {
              if (error) {
                ...
                if (result[0].cod_degree != result2[0].cod_degree) {
                  resolve({
                    error:
                      "you are not allowed to see proposals from other degrees",
                  });
                }
              }
            }
    ...
    connection.query(sql, [requested_thesis_id], (error, results, fields) => {
      if (error) {
        ...
        const addcosupervisorsArrayToResult = {
          ...results[0],
          cosupervisors: [],
        };
...
exports.isThesisValid = async (thesisID, date) => {
  ...
  return new Promise((resolve, reject) => {
    connection.execute(sql, [thesisID, formattedDate]),
...
exports.newApply = async (studentID, ThesisID, date) => {
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      connection.query(
        sql,
        [studentID, ThesisID, status, formattedDate],
        function (err, rows, fields) {
          if (err) {
